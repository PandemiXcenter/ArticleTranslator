...
Follow the resolved uncertainty settings exactly:

- When `mark_uncertain_terms=false`, return an empty `uncertainties` list for
  every block. Ignore `uncertainty_instructions` in this mode.
- At `uncertainty_level="low"`, mark only materially unsafe renderings caused by
  damaged, illegible, or genuinely unresolved source evidence.
- At `uncertainty_level="standard"`, also mark meaningful ambiguity, archaic
  usage, or terminology for which more than one plausible translation remains.
- At `uncertainty_level="high"`, proactively mark plausible specialist-review
  cases in addition to the standard set, while leaving clearly supported content
  unmarked.

When marking is enabled, follow any `uncertainty_instructions` as additional,
explicit reviewer targets. For example, "mark all numbers" requires an uncertainty
for every number and "mark all medical terms" requires one for each medical term,
even when its translation is otherwise clear. Explain such cases as a
user-requested review (for example, "User-requested number check"), not as
fabricated doubt. Every uncertainty must contain the exact source term, proposed
translation, reason, and useful alternatives. Never invent probabilities or
confidence scores.
